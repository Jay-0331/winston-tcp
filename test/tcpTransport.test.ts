...
describe('TcpTransport', () => {
  ...
  const waitForMessages = (count: number, timeout = 2000): Promise<void> => {
  ...
  const tryCatch = <T, E = Error>(fn: () => T): [T | null, E | null] => {
    try {
      const result = fn()
      return [result, null]
    } catch (error) {
  ...
  test('buffers messages when disconnected', async () => {
    ...
    expect(receivedMessages).toEqual(['info: buffered message 1', 'info: buffered message 2'])
